...
import { Server, Database, Activity, Settings, Plus, Trash2, RefreshCw, Users, Zap } from 'lucide-react';
    ...
    try {
      ...
      if (metricsData.success) {
        ...
        const processedMetrics = {
          ...
          cache: {
            ...metricsData.cache,
            hitRate: metricsData.cache?.hitRate || 
                    calculateHitRate(metricsData.cache?.hits || 0, metricsData.cache?.misses || 0),
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <h1 className="text-xl font-bold text-gray-900">Distributed Cache System</h1>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Cache Size</dt>
                  <dd className="text-lg font-medium text-gray-900">
                    {metrics.cache?.size || 0} / {metrics.cache?.capacity || 100}
                  </dd>
                </dl>
              </div>
            </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Connected Nodes</dt>
                  <dd className="text-lg font-medium text-gray-900">
...
// Cache Tab Component
const CacheTab = ({ cacheItems, onAddItem, onDeleteItem, onClearCache, newKey, setNewKey, newValue, setNewValue }) => {
  return (
    <div className="space-y-6">
      {/* Add New Item Form */}
      <div className="bg-gray-50 p-4 rounded-lg">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Add New Cache Entry</h3>
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-medium text-gray-900">Cache Entries ({cacheItems.length})</h3>
          <button
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {cacheItems.map((item) => (
                  <tr key={item.key}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{item.accessCount || 0}</td>
...
const SyncTab = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div className="text-lg font-medium text-gray-900">{syncStatus.nodeId || 'N/A'}</div>
          </div>
          <div>
            <div className="text-sm text-gray-500">Connected Nodes</div>
            <div className="text-lg font-medium text-gray-900">{syncStatus.connectedNodes || 0}</div>
          </div>
          <div>
            <div className="text-sm text-gray-500">Sync Status</div>
            <div className={`text-lg font-medium ${syncStatus.syncEnabled ? 'text-green-600' : 'text-red-600'}`}>
              {syncStatus.syncEnabled ? 'Enabled' : 'Disabled'}
            </div>
          </div>
          <div>
            <div className="text-sm text-gray-500">Total Sync Operations</div>
            <div className="text-lg font-medium text-gray-900">{syncStatus.totalSyncOperations || 0}</div>
          </div>
        </div>
      ...
      {/* Sync History */}
      <div>
        <h3 className="text-lg font-medium text-gray-900 mb-4">Recent Sync Operations</h3>
        {syncHistory.length === 0 ? (
          ...
          <div className="space-y-2">
            {syncHistory.map((operation, index) => (
              <div key={operation.id || index} className="bg-white p-4 rounded-lg border">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="text-sm text-gray-500">
                      ...
                      {operation.value && ` | Value: ${JSON.stringify(operation.value)}`}
                    </div>
                  </div>
                  <div className="text-sm text-gray-500">
...
const MetricsTab = ({ metrics }) => {
  return (
    <div className="space-y-6">
      {/* Cache Metrics */}
      <div className="bg-gray-50 p-4 rounded-lg">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Cache Metrics</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            <div className="text-sm text-gray-500">Strategy</div>
            <div className="text-lg font-medium text-gray-900">{metrics.cache?.strategy || 'LRU'}</div>
          </div>
          <div>
            <div className="text-sm text-gray-500">Capacity</div>
            <div className="text-lg font-medium text-gray-900">{metrics.cache?.capacity || 100}</div>
          </div>
          <div>
            <div className="text-sm text-gray-500">Current Size</div>
            <div className="text-lg font-medium text-gray-900">{metrics.cache?.size || 0}</div>
          </div>
          <div>
            <div className="text-sm text-gray-500">Hit Rate</div>
            <div className="text-lg font-medium text-green-600">{metrics.cache?.hitRate || 0}%</div>
          </div>
          <div>
            <div className="text-sm text-gray-500">Total Hits</div>
            <div className="text-lg font-medium text-green-600">{metrics.cache?.hits || 0}</div>
          </div>
          <div>
            <div className="text-sm text-gray-500">Total Misses</div>
            <div className="text-lg font-medium text-red-600">{metrics.cache?.misses || 0}</div>
          </div>
          <div>
            <div className="text-sm text-gray-500">Evictions</div>
            <div className="text-lg font-medium text-yellow-600">{metrics.cache?.evictions || 0}</div>
          </div>
        </div>
      </div>
      ...
      {/* Server Metrics */}
      <div className="bg-gray-50 p-4 rounded-lg">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Server Metrics</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <div className="text-sm text-gray-500">Uptime</div>
            <div className="text-lg font-medium text-gray-900">
              {metrics.server?.uptime ? `${Math.floor(metrics.server.uptime / 3600)}h ${Math.floor((metrics.server.uptime % 3600) / 60)}m` : 'N/A'}
            </div>
          </div>
          <div>
            <div className="text-sm text-gray-500">Memory Usage</div>
            <div className="text-lg font-medium text-gray-900">
              {metrics.server?.memory ? `${Math.round(metrics.server.memory.heapUsed / 1024 / 1024)} MB` : 'N/A'}
            </div>
          </div>
        </div>
      </div>
